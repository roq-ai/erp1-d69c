...
import { getLandscapeAnalyses } from 'apiSdk/landscape-analyses';
...
import { LogAnalysisListPage as ShowCaseList } from 'pages/log-analyses';

function DashboardPage() {
  const { session } = useSession();

  const landscapeAnalysisStat = useSWR<PaginatedInterface<LandscapeAnalysisInterface>>(
    () => `/landscape-analyses`,
    () => getLandscapeAnalyses({ limit: 1, offset: 0 }),
  );

  const businessApplicationStat = useSWR<PaginatedInterface<BusinessApplicationInterface>>(
    () => `/business-applications`,
    () => getBusinessApplications({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Landscape Analyses',
      result: landscapeAnalysisStat?.data?.totalCount,
      isLoading: landscapeAnalysisStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Business Applications',
      result: businessApplicationStat?.data?.totalCount,
      isLoading: businessApplicationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Log Analyses',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`log-analyses`} passHref legacyBehavior>
                  <Button
